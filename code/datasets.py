...
def german_credit(include_sex = True):
    ...
    if not include_sex:
        ...
        where_0_train = np.where(is_male_train == 0)
        where_1_train = np.where(is_male_train == 1)
        is_male_train[where_0_train] = 1
        is_male_train[where_1_train] = 0

        where_0_test = np.where(is_male_test == 0)
        where_1_test = np.where(is_male_test == 1)
        is_male_test[where_0_test] = 1
        is_male_test[where_1_test] = 0
